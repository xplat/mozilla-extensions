...
Public API used by media_native_host:
    ...
    thumb_path(file_path)             thumbnail cache path (pathlib.Path), or None if
                                      the backend has no file-based cache (e.g. Windows)
...
def request(file_path):
    """Generate/fetch a thumbnail for file_path.  Returns PNG bytes on success, None on failure."""
...
def thumb_path(file_path):
    """Thumbnail cache path (pathlib.Path) for file_path, or None if the backend
    has no file-based cache (e.g. WindowsBackend)."""

...
def _file_uri(file_path):
    """Return a file:// URI using GLib-compatible percent-encoding.

    pathlib.Path.as_uri() encodes '+' as '%2B', but GLib/GIO (and therefore
    Nautilus, Tumbler, and every other GLib-based tool) treats '+' as an
    unreserved sub-delimiter in path components and leaves it bare.  Using
    different encoding produces a different MD5 and therefore a different
    XDG thumbnail cache path, so thumbnails generated by Nautilus/Tumbler
    would never be found.  The safe set below matches the characters that
    GLib allows unencoded in URI path components (RFC 3986 sub-delimiters
    plus ':', '@', '/').
    """
...
class ThumbnailBatcher:
    ...
    def request(self, uri, timeout=30.0):
    ...
    def queue_preemptive(self, uris, mimes):
        ...
        _tumbler_queue(uris, mimes, 'background')
    ...
    def _flush(self):
        ...
        handle = _tumbler_queue(batch_uris, mimes, 'default')
...
def _tumbler_queue(uris, mimes, scheduler):
    """Call Tumbler Queue via a fresh D-Bus connection. Returns handle (int) or None."""
    try:
        ...
        try:
            ...
            return reply.body[0]   # uint32 handle
        finally:
            conn.close()
    except Exception:
        return None
...
def _start_tumbler_signal_listener(batcher):
    ...
    def _listener():
        try:
            ...
            # No 'sender=' filter: if Tumbler has not yet been activated when
            # AddMatch is sent, the daemon does not know Tumbler's unique bus
            # name and the filter would silently match nothing.  Filtering by
            # interface and path is sufficient and is activation-order-safe.
            match_str = (
                "type='signal',"
                "interface='org.freedesktop.thumbnails.Thumbnailer1',"

    ...
    if PLATFORM == 'Linux':
        if _tumbler_batcher is not None and not _is_thumb_failed(file_path):
            ...
            if _tumbler_batcher.request(uri):
                # Ready signal arrived — verify the file actually landed.
...
# them.  A separate daemon thread listens for Tumbler's Ready and Error D-Bus
# signals and resolves the waiting per-URI WaitSlots.  Finished is ignored —
# the per-request timeout handles any URIs Tumbler never reports.
#
# Signals are matched by URI, not by handle, so no handle state is needed.
# Each Queue call opens and closes its own D-Bus connection (matching the
# behaviour of the original single-request implementation that is known to
# work) so there is no shared connection state to go wrong.
#
# Preemptive directory-queue calls (scheduler='background') are fire-and-forget
# and do not involve WaitSlots.
...
class ThumbnailBatcher:
    BATCH_WINDOW = 0.05   # seconds to accumulate requests before flushing
    ...
    def request(self, uri, timeout=8.0):
        """Add uri to the next batch and block until Tumbler signals or timeout."""
        with self._lock:
            slot = _WaitSlot()
    ...
    def queue_preemptive(self, uris, mimes):
        """Fire-and-forget: queue uris with scheduler='background'."""
        _tumbler_queue_once(uris, mimes, 'background')

    def on_ready(self, uris):
        with self._lock:
            for uri in uris:
                self._resolve(uri, True)

    def on_error(self, uris):
        with self._lock:
            for uri in uris:
                self._resolve(uri, False)

    def _resolve(self, uri, success):
    ...
    def _flush(self):
        with self._lock:
            batch_uris = list(self._pending.keys())
            self._timer = None
        if not batch_uris:
            return
        mimes = [
        ...
        if not _tumbler_queue_once(batch_uris, mimes, 'default'):
            # Queue call failed — wake all waiting slots immediately.
            with self._lock:
                for uri in batch_uris:
                    self._resolve(uri, False)


def _tumbler_queue_once(uris, mimes, scheduler):
    """Open a fresh D-Bus connection, call Tumbler Queue, close. Returns True on success."""
    try:
...
def _start_tumbler_signal_listener(batcher):
    """Daemon thread: subscribe to Tumbler signals and feed them to batcher."""
    def _listener():
        try:
            from jeepney import DBusAddress, new_method_call   # type: ignore
            ...
            conn      = open_dbus_connection(bus='SESSION')
            dbus_addr = DBusAddress('/org/freedesktop/DBus',
            ...
            while True:
                ...
                try:
                    if member == 'Ready':
                        _handle, uris = msg.body
                        batcher.on_ready(list(uris))
                    elif member == 'Error':
                        _handle, failed_uris = msg.body[0], msg.body[1]
                        batcher.on_error(list(failed_uris))
                    # Finished intentionally ignored — timeout handles residuals.
                except Exception:
                    pass
        except Exception:
            pass
...
def _init_tumbler_batcher():
    """Initialise the global ThumbnailBatcher and signal listener (Linux only)."""
